...
parser.add_argument('--network', type=str, default='sphere20',
                    help='Which network for train. (sphere20, sphere36, sphere64, mobile)')
# Classifier
...
def main():
    ...
    if args.network == 'sphere20':
        model = sphere20(embedding_dim=512, in_channels=1 if args.is_gray else 3)
        model_eval = sphere20(embedding_dim=512, in_channels=1 if args.is_gray else 3)
    elif args.network == 'sphere36':
        model = sphere36(embedding_dim=512, in_channels=1 if args.is_gray else 3)
        model_eval = sphere36(embedding_dim=512, in_channels=1 if args.is_gray else 3)
    elif args.network == 'sphere64':
        model = sphere64(embedding_dim=512, in_channels=1 if args.is_gray else 3)
        model_eval = sphere64(embedding_dim=512, in_channels=1 if args.is_gray else 3)
    ...
    save_path = os.path.join(args.save_path, f'{args.network}_0_checkpoint.pth')
    ...
    classifier = {
        ...
        'ARC': layer.ArcFace(512, args.num_class).to(device),
